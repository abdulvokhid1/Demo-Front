...
const MemberLookup = () => {
    ...
    const [userState, setUserState] = useState<IdNameStateType[]>([])
        ...
        if (userListState?.length > 0) {
            return (
                userListState.map((item, index) => {
                    return (
                        <tr key={item.id} className="selectToggle" style={{cursor: 'pointer'}}>
                            <td>{index+1}</td>
                            <td>
                                <input id="isRelation" type="checkbox" name="id" value={item.id} onChange={(e) => {
                                    if (e.target.checked) {
                                        const newUser = {
                                            id: item.id || 0,
                                            name: item.name || '',
                                        ...
                                        userState.map((user: IdNameStateType, i: number) => {user.id == item.id? indices.push(i) : noop})
                                        if (indices?.length) {
                                            indices.map((index1: any, index: number) => {
                                                userState.splice(index1, 1)
                            ...
                            <td style={{textAlign: 'left'}}>{item.recom?.name || ''}</td>
                        </tr>
                    )
            ...
            <div className="content_section">
                <div className="content_section_fix" style={{background: '#fff'}}>
                    <form name="relationForm" method="get" action={onSubmit}>
                        ...
                        <input type="hidden" name="q1" value="mode=&amp;app_mode=popup"/>
                        ...
                        <div className="form_box_area">
                            <table className="form_TB" summary="검색항목">
                                ...
                                <tr>
                                    ...
                                    <td className="conts">
                                        <input type="text" name="pass_id" className="input_text"/>
                                    </td>
                    ...
                    <div className="content_section_inner">
                        <table className="list_TB" summary="리스트기본">
                            ...
                            <tr>
                                ...
                                <th scope="col" className="colorset">회원명(대표자)</th>
                                <th scope="col" className="colorset">상호명</th>

                                <th scope="col" className="colorset">등급</th>
                                <th scope="col" className="colorset">추천인</th>
                    ...
                    <div className="top_btn_area">
                        <div className="btn_line_up_center">
                            <span className="shop_btn_pack btn_input_blue">
                                <input type="button" className="input_medium" title="선택적용" value="선택적용"
                                       onClick={() => {
                                           const idList: number[] = userState.map((item, index) => (item.id))
                                           const nameList: string[] = userState.map((item, index) => (item.name))
                                           const userid_hidden = window.opener.document.getElementById('userid_hidden');
                                           userid_hidden.value = idList.join('-');
                                           const pointIDArray = window.opener.document.getElementById('pointIDArray');
                                           pointIDArray.value = nameList.join('   ');
                    ...
                    <span className="shop_btn_pack">
                            ...
                            <span className="shop_btn_pack btn_input_blue">
                                <input type="button" className="input_medium" title="창닫기" value="창닫기"
                                       onClick={() => {
                                           window.opener.focus();
                                           window.close();
                                       }}/>
